...
namespace UkTote.Tests
{
    ...
    public class UkToteBetTests
    {
        ...
        private const string Username = "ATCentrum6";
        ...
        {
            var reply = await _gateway.SellBet(DateTime.UtcNow, 1, 1, 200, 200, Enums.BetCode.Swinger, Enums.BetOption.Straight, new[] { 1, 2 });
            Assert.IsTrue(!string.IsNullOrEmpty(reply.Tsn));
        }
        ...
        {
            var reply = await _gateway.PayEnquiry("0000C8293DD06674");
        ...
        public async Task RunBetfredBetPlacement()
        {
            ...
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 4, 200, 200, Enums.BetCode.Exacta, Enums.BetOption.Straight, new[] { 117, 218 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    2   Exacta  10  10  1   Straight    6,7
            betStr = "//Bath    2   Exacta  10  10  1   Straight    6,7";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 2, 1000, 1000, Enums.BetCode.Exacta, Enums.BetOption.Straight, new[] { 106, 207 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Exacta  8   8   1   Straight    9,10
            betStr = "//Bath    1   Exacta  8   8   1   Straight    9,10";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 800, 800, Enums.BetCode.Exacta, Enums.BetOption.Straight, new[] { 109, 210 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Exacta  5   10  2   Permutation 2,3
            betStr = "//Bath    1   Exacta  5   10  2   Permutation 2,3";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 500, 1000, Enums.BetCode.Exacta, Enums.BetOption.Permutation, new[] { 2, 3 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    2   Exacta  20  40  2   Permutation 6,7
            betStr = "//Bath    2   Exacta  20  40  2   Permutation 6,7";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 2, 2000, 4000, Enums.BetCode.Exacta, Enums.BetOption.Permutation, new[] { 6, 7 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   2   Exacta  25  50  2   Permutation 27,28
            betStr = "//Ascot   2   Exacta  25  50  2   Permutation 27,28";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 2, 2500, 5000, Enums.BetCode.Exacta, Enums.BetOption.Permutation, new[] { 27, 28 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Exacta  2   12  6   Permutation 2,3,4
            betStr = "//Bath    1   Exacta  2   12  6   Permutation 2,3,4";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 200, 1200, Enums.BetCode.Exacta, Enums.BetOption.Permutation, new[] { 2, 3, 4 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    3   Exacta  5   30  6   Permutation 8,9,10
            betStr = "//Bath    3   Exacta  5   30  6   Permutation 8,9,10";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 3, 500, 3000, Enums.BetCode.Exacta, Enums.BetOption.Permutation, new[] { 8, 9, 10 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Exacta  15  90  6   Permutation 8,9,10
            betStr = "//Bath    1   Exacta  15  90  6   Permutation 8,9,10";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 1500, 9000, Enums.BetCode.Exacta, Enums.BetOption.Permutation, new[] { 8, 9, 10 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   2   Exacta  10  60  6   Permutation 4,5,28
            betStr = "//Ascot   2   Exacta  10  60  6   Permutation 4,5,28";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 2, 1000, 6000, Enums.BetCode.Exacta, Enums.BetOption.Permutation, new[] { 4, 5, 28 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   1   Exacta  2   4   2   Banker  901,2,3
            betStr = "//Ascot   1   Exacta  2   4   2   Banker  901,2,3";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 1, 200, 400, Enums.BetCode.Exacta, Enums.BetOption.Banker, new[] { 901, 2, 3 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   1   Exacta  4   8   2   Banker  902,1,3
            betStr = "//Ascot   1   Exacta  4   8   2   Banker  902,1,3";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 1, 400, 800, Enums.BetCode.Exacta, Enums.BetOption.Banker, new[] { 902, 1, 3 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   4   Exacta  7.5 15  2   Banker  917,17,18,19
            betStr = "//Ascot   4   Exacta  7.5 15  2   Banker  917,17,18,19";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 4, 750, 1500, Enums.BetCode.Exacta, Enums.BetOption.Banker, new[] { 917, 17, 18, 19 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   1   Trifecta    2   2   1   Straight    1,2,3
            betStr = "//Ascot   1   Trifecta    2   2   1   Straight    1,2,3";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 1, 200, 200, Enums.BetCode.Trifecta, Enums.BetOption.Straight, new[] { 101, 202, 303 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   1   Trifecta    5   5   1   Straight    3,4,5
            betStr = "//Ascot   1   Trifecta    5   5   1   Straight    3,4,5";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 1, 500, 500, Enums.BetCode.Trifecta, Enums.BetOption.Straight, new[] { 103, 204, 305 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   2   Trifecta    10  10  1   Straight    6,7,27
            betStr = "//Ascot   2   Trifecta    10  10  1   Straight    6,7,27";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 2, 1000, 1000, Enums.BetCode.Trifecta, Enums.BetOption.Straight, new[] { 106, 207, 327 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Trifecta    20  480 24  Permutation 1,2,3,5
            betStr = "//Bath    1   Trifecta    20  480 24  Permutation 1,2,3,5";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 2000, 48000, Enums.BetCode.Trifecta, Enums.BetOption.Permutation, new[] { 1, 2, 3, 5 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    5   Trifecta    2   48  24  Permutation 2,4,6,8
            betStr = "//Bath    5   Trifecta    2   48  24  Permutation 2,4,6,8";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 5, 200, 4800, Enums.BetCode.Trifecta, Enums.BetOption.Permutation, new[] { 2, 4, 6, 8 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Trifecta    6   36  6   Permutation 2,4,10
            betStr = "//Bath    1   Trifecta    6   36  6   Permutation 2,4,10";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 600, 3600, Enums.BetCode.Trifecta, Enums.BetOption.Permutation, new[] { 2, 4, 10 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Trifecta    10  240 24  Permutation 1,2,3,10
            betStr = "//Bath    1   Trifecta    10  240 24  Permutation 1,2,3,10";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 1000, 24000, Enums.BetCode.Trifecta, Enums.BetOption.Permutation, new[] { 1, 2, 3, 10 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   6   Trifecta    20  120 6   Banker  901,2,3,4
            betStr = "//Ascot   6   Trifecta    20  120 6   Banker  901,2,3,4";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 6, 2000, 12000, Enums.BetCode.Trifecta, Enums.BetOption.Banker, new[] { 901, 2, 3, 4 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   6   Trifecta    5   30  6   Banker  902,1,3,4
            betStr = "//Ascot   6   Trifecta    5   30  6   Banker  902,1,3,4";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 6, 500, 3000, Enums.BetCode.Trifecta, Enums.BetOption.Banker, new[] { 902, 1, 3, 4 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Trifecta    10  60  6   Banker  903,1,2,3,4
            betStr = "//Bath    1   Trifecta    10  60  6   Banker  903,1,2,3,4";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 1000, 6000, Enums.BetCode.Trifecta, Enums.BetOption.Banker, new[] { 903, 1, 2, 3, 4 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);
            ...
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 4, 2000, 2000, Enums.BetCode.Swinger, Enums.BetOption.Straight, new[] { 17, 19 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   1   Swinger 10  10  1   Straight    2,6
            betStr = "//Ascot   1   Swinger 10  10  1   Straight    2,6";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 1, 1000, 1000, Enums.BetCode.Swinger, Enums.BetOption.Straight, new[] { 2, 6 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Swinger 20  20  1   Straight    2,10
            betStr = "//Bath    1   Swinger 20  20  1   Straight    2,10";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 2000, 2000, Enums.BetCode.Swinger, Enums.BetOption.Straight, new[] { 2, 10 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    3   Swinger 2   6   3   Permutation 10,11,13
            betStr = "//Bath    3   Swinger 2   6   3   Permutation 10,11,13";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 3, 200, 600, Enums.BetCode.Swinger, Enums.BetOption.Permutation, new[] { 10, 11, 13 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    5   Swinger 50  150 3   Permutation 2,4,6
            betStr = "//Bath    5   Swinger 50  150 3   Permutation 2,4,6";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 5, 5000, 15000, Enums.BetCode.Swinger, Enums.BetOption.Permutation, new[] { 2, 4, 6 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   2   Swinger 2   6   3   Permutation 25,26,27
            betStr = "//Ascot   2   Swinger 2   6   3   Permutation 25,26,27";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 2, 200, 600, Enums.BetCode.Swinger, Enums.BetOption.Permutation, new[] { 25, 26, 27 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Bath    1   Swinger 20  60  3   Permutation 1,3,10
            betStr = "//Bath    1   Swinger 20  60  3   Permutation 1,3,10";
            reply = await _gateway.SellBet(DateTime.UtcNow, 2, 1, 2000, 6000, Enums.BetCode.Swinger, Enums.BetOption.Permutation, new[] { 1, 3, 10 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   4   Swinger 2   6   3   Banker  918,16,17,19
            betStr = "//Ascot   4   Swinger 2   6   3   Banker  918,16,17,19";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 4, 200, 600, Enums.BetCode.Swinger, Enums.BetOption.Banker, new[] { 918, 16, 17, 19 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   4   Swinger 10  30  3   Banker  917,15,16,18
            betStr = "//Ascot   4   Swinger 10  30  3   Banker  917,15,16,18";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 4, 1000, 3000, Enums.BetCode.Swinger, Enums.BetOption.Banker, new[] { 917, 15, 16, 18 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);

            //Ascot   2   Swinger 12  24  2   Banker  905,4,5,6
            betStr = "//Ascot   2   Swinger 12  24  2   Banker  905,4,5,6";
            reply = await _gateway.SellBet(DateTime.UtcNow, 1, 2, 1200, 2400, Enums.BetCode.Swinger, Enums.BetOption.Banker, new[] { 905, 4, 5, 6 });
            _logger.InfoFormat("{0} : {1} : {2} : {3}", reply.Tsn, reply.BetId, betStr, reply.ErrorText);
        ...
        public async Task RunBetfredPaymentRequest()
        {
            ...
            {
                var reply = await _gateway.PayEnquiry(tsn);
