...
namespace UkTote
{
    public partial class ToteGateway : CancellableQueueWorker<MessageBase>, IDisposable, IToteGateway
    {
        ...
        private readonly Dictionary<Tuple<Enums.MessageType, Enums.ActionCode>, Type> _lookup = new();
        private readonly Dictionary<Enums.MessageType, bool> _ignoreUpdates = new();
        private WatchdogTimer? _watchdogTimer;
        private readonly int _watchdogTimeoutMs;
        private readonly ConcurrentDictionary<int, Guid> _refLookup = new();
        ...
        private void ProcessPacket(Header header, byte[] buffer)
        {
            ...
            if (_lookup.ContainsKey(key))
            {
                ...
                {
                    OnLoginError?.Invoke((AccountLoginError)packet);
                }
                else if (pType == typeof(AccountLoginSuccess))
                {
                    OnLoginSuccess?.Invoke((AccountLoginSuccess)packet);
                }
                else if (pType == typeof(AccountLogoutError))
                {
                    OnLogoutError?.Invoke((AccountLogoutError)packet);
                }
                else if (pType == typeof(AccountLogoutSuccess))
                {
                    OnLogoutSuccess?.Invoke((AccountLogoutSuccess)packet);
                }
                else if (pType == typeof(MeetingEndDateErrorReply))
                {
                    OnMeetingEndDateError?.Invoke((MeetingEndDateErrorReply)packet);
                }
                else if (pType == typeof(MeetingEndDateReply))
                {
                    OnMeetingEndDate?.Invoke((MeetingEndDateReply)packet);
                }
                else if (pType == typeof(MeetingReply))
                {
                    OnMeeting?.Invoke((MeetingReply)packet);
                }
                else if (pType == typeof(RacecardReply))
                {
                    OnRacecard?.Invoke((RacecardReply)packet);
                }
                else if (pType == typeof(RacePoolReply))
                {
                    OnRacePool?.Invoke((RacePoolReply)packet);
                }
                else if (pType == typeof(RaceReply))
                {
                    OnRace?.Invoke((RaceReply)packet);
                }
                else if (pType == typeof(RunnerReply))
                {
                    OnRunner?.Invoke((RunnerReply)packet);
                }
                else if (pType == typeof(MeetingPoolReply))
                {
                    OnMeetingPool?.Invoke((MeetingPoolReply)packet);
                }
                else if (pType == typeof(SellBetFailed))
                {
                    OnSellBetFailed?.Invoke((SellBetFailed)packet);
                }
                else if (pType == typeof(SellBetSuccess))
                {
                    OnSellBetSuccess?.Invoke((SellBetSuccess)packet);
                }
                else if (pType == typeof(PayEnquirySuccess))
                {
                    OnPayEnquirySuccess?.Invoke((PayEnquirySuccess)packet);
                }
                else if (pType == typeof(PayEnquiryFailed))
                {
                    OnPayEnquiryFailed?.Invoke((PayEnquiryFailed)packet);
                }
                else if (pType == typeof(RaceUpdate))
                {
                    OnRaceUpdate?.Invoke((RaceUpdate)packet);
                }
                else if (pType == typeof(MeetingPoolUpdate))
                {
                    OnMeetingPoolUpdate?.Invoke((MeetingPoolUpdate)packet);
                }
                else if (pType == typeof(RacePoolUpdate))
                {
                    OnRacePoolUpdate?.Invoke((RacePoolUpdate)packet);
                }
                else if (pType == typeof(PoolSubstituteUpdate))
                {
                    OnPoolSubstituteUpdate?.Invoke((PoolSubstituteUpdate)packet);
                }
                else if (pType == typeof(MeetingUpdate))
                {
                    OnMeetingUpdate?.Invoke((MeetingUpdate)packet);
                }
                else if (pType == typeof(MeetingSalesUpdate))
                {
                    OnMeetingSalesUpdate?.Invoke((MeetingSalesUpdate)packet);
                }
                else if (pType == typeof(RaceSalesUpdate))
                {
                    OnRaceSalesUpdate?.Invoke((RaceSalesUpdate)packet);
                }
                else if (pType == typeof(MeetingPoolSalesUpdate))
                {
                    OnMeetingPoolSalesUpdate?.Invoke((MeetingPoolSalesUpdate)packet);
                }
                else if (pType == typeof(RacePoolSalesUpdate))
                {
                    OnRacePoolSalesUpdate?.Invoke((RacePoolSalesUpdate)packet);
                }
                else if (pType == typeof(ResultUpdate))
                {
                    OnResultUpdate?.Invoke((ResultUpdate)packet);
                }
                else if (pType == typeof(SubstituteUpdate))
                {
                    OnSubstituteUpdate?.Invoke((SubstituteUpdate)packet);
                }
                else if (pType == typeof(WeighedInUpdate))
                {
                    OnWeighedInUpdate?.Invoke((WeighedInUpdate)packet);
                }
                else if (pType == typeof(EndOfRacingUpdate))
                {
                    OnEndOfRacingUpdate?.Invoke((EndOfRacingUpdate)packet);
                }
                else if (pType == typeof(MsnReply))
                {
                    OnMsnReply?.Invoke((MsnReply)packet);
                }
                else if (pType == typeof(CurrentMsnReply))
                {
                    OnCurrentMsnReply?.Invoke((CurrentMsnReply)packet);
                }
                else if (pType == typeof(RacePoolDividendUpdate))
                {
                    OnRacePoolDividendUpdate?.Invoke((RacePoolDividendUpdate)packet);
                }
                else if (pType == typeof(MeetingPoolWillPayUpdate))
                {
                    OnMeetingPoolWillPayUpdate?.Invoke((MeetingPoolWillPayUpdate)packet);
                }
                else if (pType == typeof(RaceWillPayUpdate))
                {
                    OnRaceWillPayUpdate?.Invoke((RaceWillPayUpdate)packet);
                }
                else if (pType == typeof(RaceExtendedWillPayUpdate))
                {
                    OnRaceExtendedWillPayUpdate?.Invoke((RaceExtendedWillPayUpdate)packet);
                }
                else if (pType == typeof(LegBreakdownUpdate))
                {
                    OnLegBreakdownUpdate?.Invoke((LegBreakdownUpdate)packet);
                }
                else if (pType == typeof(MeetingPoolTotalUpdate))
                {
                    OnMeetingPoolTotalUpdate?.Invoke((MeetingPoolTotalUpdate)packet);
                }
                else if (pType == typeof(ComplexRacePoolTotalUpdate))
                {
                    OnComplexRacePoolTotalUpdate?.Invoke((ComplexRacePoolTotalUpdate)packet);
                }
                else if (pType == typeof(RunnerUpdate))
                {
                    OnRunnerUpdate?.Invoke((RunnerUpdate)packet);
                }
                else if (pType == typeof(CurrentBalanceReply))
                {
                    OnCurrentBalanceReply?.Invoke((CurrentBalanceReply)packet);
                }
                else if (pType == typeof(MeetingPoolDividendUpdate))
                {
                    OnMeetingPoolDividendUpdate?.Invoke((MeetingPoolDividendUpdate)packet);
                }
                else if (pType == typeof(SuperComplexPoolDividendUpdate))
                {
                    OnSuperComplexPoolDividendUpdate?.Invoke((SuperComplexPoolDividendUpdate)packet);
                }
                else if (pType == typeof(MatrixPoolDividendUpdate))
                {
                    OnMatrixPoolDividendUpdate?.Invoke((MatrixPoolDividendUpdate)packet);
                }
                else if (pType == typeof(ComplexRacePoolDividendUpdate))
                {
                    OnComplexRacePoolDividendUpdate?.Invoke((ComplexRacePoolDividendUpdate)packet);
                }
                else if (pType == typeof(MeetingPayUpdate))
                {
                    OnMeetingPayUpdate?.Invoke((MeetingPayUpdate)packet);
                }
                else if (pType == typeof(RacePayUpdate))
                {
                    OnRacePayUpdate?.Invoke((RacePayUpdate)packet);
                }
                else if (pType == typeof(MeetingPoolPayUpdate))
                {
                    OnMeetingPoolPayUpdate?.Invoke((MeetingPoolPayUpdate)packet);
                }
                else if (pType == typeof(RacePoolPayUpdate))
                {
                    OnRacePoolPayUpdate?.Invoke((RacePoolPayUpdate)packet);
                }
            }
            else
            {
            ...
            Enums.BetCode betCode, Enums.BetOption betOption, Selection[]? selections, int? useBetId)
        {
            ...
            var req = new SellBetRequest()
            {
                ...
                BetId = (uint)betId,
                BetOption = betOption,
                NumberOfSelections = (ushort)(selections?.Length??0),
        ...
        public Task<IList<PayEnquiryReply?>> PayEnquiryBatch(IList<string> tsnList)
        {
            ...
            successHandler += (reply) => 
            {
                var tsn = reply?.Tsn?.Replace("\0", string.Empty)??string.Empty;
                ...
                if (responses.ContainsKey(tsn))
                {
                    ...
                    {
                        Tsn = tsn,
                        PayoutAmount = reply?.PayoutAmount??0,
                        VoidAmount = reply?.VoidAmount??0,
                        ErrorCode = Enums.ErrorCode.Success,
                        ErrorText = string.Empty
                    };
